...
class FeatureExtractor():
    ...
    def _compute_frequencies(self, audio_buffer):
        ...
        top_8 = np.argsort(f)[-8:]

        return np.append(out, top_8)
    ...
    def _compute_formant_features(self, window):
        ...
        #print(type(formants), formants)

        hist = np.histogram(formants, bins = 11, range = (0,5500))
    ...
    if pitches:
        ...
        def _compute_pitch_features(self, window):
            ...
            # with some quick testing I got that pitches with confidence over 1/4 can range from -12 to 110

            hist = np.histogram(pitch_contour, bins = 16, range = (-20,120), weights = pitch_confidence)

            mean_pitch = 0.
            denom = 0.
            for i, pitch in enumerate(pitch_contour):
                mean_pitch += pitch * pitch_confidence[i]
                denom += pitch_confidence[i]

            return np.append(hist[0], [mean_pitch / denom])
    ...
    def _compute_delta_coefficients(self, window, n=2):
        ...
        mfcc_deltas = self._compute_deltas(mfcc_feats, n)

        if not self.include_delta_deltas:
            return np.ndarray.flatten(mfcc_deltas)
        
        delta_deltas = self._compute_deltas(mfcc_deltas, n)

        return np.append(np.ndarray.flatten(mfcc_deltas), np.ndarray.flatten(delta_deltas))

    def _compute_deltas(self, feats, n=2):
        ...
        for t in range(n, n_frames - n):
            deltas[t-n] = np.sum(float(n) * feats[t+1:t+n+1], axis=0) - np.sum(float(n) * feats[t-n:t], axis=0)
